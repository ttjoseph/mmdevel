...
def trim_correl_matrix(pairs, correl, cutoff):
    ...
    for i in xrange(len(pairs)):
        # Check the ith row of correlation matrix.
        # The correlation matrix should be symmetric, so we can get
        # away with this.
        if len(numpy.where(correl[i,:] < cutoff):
            # Do stuff!
            pass
    return (pairs, correl)
...
if __name__ == "__main__":
    ...
    correl = numpy.loadtxt("correl.txt", correl, fmt="%11.6G ")
