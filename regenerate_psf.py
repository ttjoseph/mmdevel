...
"""
    ...
    if args.patch is not None or args.mutate is not None:
        ...
        #psfgen_script += """# Delete leftover atoms that should no longer be there after patching
        #foreach segid []
        #"""
...
"""
    ...
    elif 'MOLECULE DESTROYED' in str(c.stdout, 'utf-8'):
        print(f'Error: VMD psfgen had an error. Take a look at the VMD output file.', file=sys.stderr)
        should_renumber = False
    ...
    # Finally, check for electroneutrality
    psf = ShadyPSF(out_psf_filename)
    total_charge = 0.0
    for a in psf.atoms:
        total_charge += float(a[ShadyPSF.CHARGE])
    print(f'Total charge is {total_charge}.', file=sys.stderr)
    if fabs(total_charge) > 0.01:
        print(f'WARNING: Total charge is not really close to zero! That is bad! So I am renaming your output PSF to {out_psf_filename}.nonzero_charge', file=sys.stderr)
        shutil.move(out_psf_filename, f'{out_psf_filename}.nonzero_charge')

    print(f'All done. Do you need to regenerate the equilibration restraints, perhaps using generate_charmmgui_*_restraints in libttj.vmd.tcl?', file=sys.stderr)
