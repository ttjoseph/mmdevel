...
func main() {
	...
	switch command {
	case "dump-mdouts":
		main_dumpMdoutsToBin(filename)
	case "project-matrix":
		main_projectMatrix()
	case "calc-correl":
		main_calcCorrel(filename)
	default:
		flag.Usage()
	}
}
...
func main_dumpMdoutsToBin(outFilename string) {
	...
	for _, v := range (names) {
		if strings.HasSuffix(v, ".mdout.gz") {
			filenames.Push(dirname + v)
		}
	}
	// Output file
...
func projectCorrelSingleResiduePair(correl []float32, pairs *vector.Vector, numResidues, i, j int, result []float32, ch chan int) {
	...
	for ij := 0; ij < numPairs; ij++ {
		...
		for kl := 0; kl < ij; kl++ {
			...
			if ((pij.Row == i || pij.Col == i) && (pkl.Row == j || pkl.Col == j)) || ((pij.Row == j || pij.Col == j) && (pkl.Row == i || pkl.Col == i)) {
	...
	if j == 0 {
		fmt.Fprintf(os.Stderr, "projectCorrelSingleResiduePair done: %d. %s\n", i, amber.Status())
	}
	ch <- 0
}
...
func CalcCorrelations(filenames []string, average []float32, pairs *vector.Vector, numResidues int) []float32 {
	...
	for _, filename := range (filenames) {
		...
		for i := 0; i < numFrames; i += BATCH_SIZE {
			...
			if numFrames-i < numFramesNow {
				numFramesNow = numFrames - i
			}
			...
			for ij := 0; ij < pairs.Len(); ij += numRowsPerBatch {
				...
				if numRowsNow > pairs.Len()-ij {
					numRowsNow = pairs.Len() - ij
				}
	...
	for i := 0; i < len(num); i++ {
		correl[i] = float32(num[i] / denom[i])
	}
	return correl
}
...
// ij = [ij_a, ij_b)
func calcCorrelationsPiece(pairsEnergies, average []float32, pairs *vector.Vector, numResidues, numFrames, ij_a, ij_b int, num, denom []float64, ch chan int) {
	numPairs := pairs.Len()
...
func AverageEnergies(filenames []string, numResidues int) []float32 {
	...
	for _, filename := range (filenames) {
		...
		for frame := 0; frame < numFrames; frame += BATCH_SIZE {
			...
			if thisBatchSize > numFrames-frame {
				thisBatchSize = numFrames - frame
			}
	...
	for i := 0; i < len(result); i++ {
		result[i] = float32(sum[i] / float64(totalNumFrames))
	}
	return result
}
