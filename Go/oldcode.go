...
func CalcCorrelationsMonolithic(pairsEnergies, average []float32, pairs *vector.Vector,
    numResidues, numFrames int) []float32 {
...
func calcCorrelationsSingleMonolithic(pairsEnergies, average []float32, pairs *vector.Vector,
        numResidues, numFrames, ij int, correl []float32, ch chan int) {
...
func AverageEnergies(energies [][]float32, numResidues int) []float32 {
    ...
    for row := 0; row < numResidues; row++ { <-ch }
    return result;
}
...
func averageEnergiesSingleRow(energies [][]float32, numResidues, row int, result []float32, ch chan int) {
    ...
    for frame := 0; frame < numFrames; frame++ {
        for i := 0; i < numResidues; i++ { result[offs+i] += energies[frame][offs+i]; }
    }
    // Divide by number of frames to get the average!
    for i := 0; i < numResidues; i++ { result[offs+i] /= float32(numFrames) }
